...
export default function VisualizerPage({ onLogout, displayName, hideUI = false }: VisualizerPageProps) {
  ...
  // Pause the Butterchurn render loop only when the document is hidden.
  // The visualizer is intentionally still mounted AND still rendering
  // behind the frosted layer on /h /e /u — that's the product, not a
  // perf bug. Don't reintroduce a hideUI-based pause here.
  useEffect(() => {
    const eng = getVisualizerEngine()
    const sync = () => eng.setPaused(document.hidden)
    sync()
