...
interface Props {
  ...
  onAnimationSpeed: (v: number) => void
}
...
function Row({ label, val, min, max, unit, cb, step: stepOverride }: { label: string; val: number; min: number; max: number; unit: string; cb: (v: number) => void; step?: number }) {
  const step = stepOverride ?? (max - min) / 100
  return (
    <div style={row}>
...
export default function VisualizerControls(p: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', overflow: 'auto', height: '100%', paddingTop: 28 }}>
      ...
      <Tooltip text="ANIMATION SPEED" detail="Controls overall animation speed (1.0x = normal)">
      <div style={{ width: '100%' }}><Row label="ANIM SPEED" val={p.animationSpeed} min={0.1} max={3} unit="x" cb={p.onAnimationSpeed} step={0.1} /></div>
      </Tooltip>
