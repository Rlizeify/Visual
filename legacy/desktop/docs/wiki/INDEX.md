...
| [[topics/spotify-integration]] | Spotify, OAuth PKCE, Web API, loopback, getDisplayMedia, setDisplayMediaRequestHandler, SpotifyPlayer, Enable Audio Reactivity, scope version, sp-reconnect-banner | 4 | 2026-04-07 (r7) | active |
...
| [[topics/persistence-media-library]] | SQLite, better-sqlite3, save/load, media library, settings, project state, video analysis, toFileURL, drive colon | 4 | 2026-04-07 (r7) | active |
| [[topics/window-architecture]] | Electron, multi-window, Hub, Cockpit, Studio, IPC, preload, tool launcher, CSP, Widevine, file: media-src, vite publicDir | 4 | 2026-04-07 (r8) | active |
| [[topics/ui-design-system]] | CSS variables, palette, dial, tooltip, tutorial, Hitmarker fonts, MHEU, theme, aesthetic, vite font bundling | 5 | 2026-04-07 (r8) | active |
...
- 2026-04-07 (r8): Incremental recompile for commit `3c96d40`. Updated `ui-design-system` and `window-architecture`. Hitmarker Text fonts were silently absent from production builds because `apps/desktop/src/styles/fonts.css` referenced them via `../../fonts/18082023_Hitmarker/Text/WOFF/...`, a path that exits `apps/desktop/src/`; with no `publicDir` set in `vite.config.ts`, Vite shipped the CSS but dropped the files. Fix: 8 referenced face files (Regular/Italic/Medium/Bold × woff2+woff) copied into `apps/desktop/src/styles/fonts/HitmarkerText/`, `fonts.css` `url()` paths rewritten to `./fonts/HitmarkerText/...` (mirroring the working `SDGlitch.ttf` pattern). `npx vite build` then emits hashed copies (e.g. `HitmarkerText-Regular-APgbd4-k.woff2`). The architectural rule — *bundled CSS `url()`s must stay inside `src/` unless `publicDir` is configured* — is now documented in both topics' Patterns sections. Original `apps/desktop/fonts/18082023_Hitmarker/` tree left in place as an archive of unused Display weights. Only `active.md` changed in source; no new topics or concepts.
- 2026-04-07 (r7): Incremental recompile for commit `73dc19a`. Updated `spotify-integration`, `window-architecture`, and `persistence-media-library`. Spotify scope version bumped `'2'`→`'3'` in `electron/spotify-auth.ts:11` to clear stale collaborator tokens (symptom: `/v1/me/playlists` returned playlists with `tracks === undefined` and `/v1/playlists/{id}/tracks` returned 403); `SpotifyBrowser.tsx` gained an `sp-reconnect-banner` when every playlist reports `trackCount === 0`; `SpotifyPlayerAPI.ts` warn log now includes `typeof p.tracks`. `apps/desktop/index.html` CSP added `file:` to both `media-src` and `img-src` so `VideoPreview.tsx` can load local videos via `file://`. `VideoPreview.toFileURL()` now restores the Windows drive-letter colon after `encodeURIComponent` (`/C%3A/` → `/C:/`). Only `active.md` changed in source; no new topics or concepts.
